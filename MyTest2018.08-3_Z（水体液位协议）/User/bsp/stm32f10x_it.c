...
vu32  freq[Filter_Times];

vu16   freq_index = 0;

bool  measure_success_flag = FALSE;
bool  gauge_timeout_flag = FALSE;

bool  freq_en  = FALSE;

extern void TIM_CallBack2(void);
...
void TIM5_IRQHandler(void) 
{
	if (TIM_GetITStatus(TIM5, TIM_IT_CC1) != RESET)
	{
		...
		if (IC2Value != 0)
		{
			Frequency = 72000000 / IC2Value;//*0.968284609;//0.9979753767;
			
			freq[freq_index++] = Frequency;	
		
			if(freq_index >= Filter_Times)	  
			{
				freq_index = 0;
				...
				measure_success_flag = TRUE;
				gauge_timeout_flag   = FALSE;
...
vu32 Timer4_Counter = 0x0;

extern u32 wait_time_gauge;
	...
	if (TIM_GetITStatus(TIM4, TIM_IT_Update) != RESET)
	{
		...
		if(freq_en)
		{
			if(Timer4_Counter++ >= 2)
			{
				...
				gauge_timeout_flag   = TRUE;
				measure_success_flag = FALSE;
